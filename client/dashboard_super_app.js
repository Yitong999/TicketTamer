...
class Supervisor {
    ...
    addSpeedChart(speedChart, ticket) {
      ...
      // Check if the speed chart already exists in the mapwo
...
class AnalysisView{
    ...
    async downloadGeneralReportCSV(tickets) {
        ...
        const completeTickets = tickets.filter(ticket => ticket.status == 'complete');

        console.log('completeTickets', completeTickets)
        // TODO: filter tickets with status != 'complete
        completeTickets.sort((a, b) => a.supervisor_name.localeCompare(b.supervisor_name));
